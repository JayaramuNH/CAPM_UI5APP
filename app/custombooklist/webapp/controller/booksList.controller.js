...
sap.ui.define([
	...
	return Controller.extend("custom.booklist.custombooklist.controller.booksList", {
		...
		onViewBookPressed: function () {
			this._HideAllPanel();
			this.byId("viewBookPanel").setVisible(true);
		},
		...
		onCreate: function () {
			...
			this.getView().getModel("appView").setProperty("/bookTitleEmpty", true);
		...
		onInputChange: function (oEvt) {
			if (oEvt.getParameter("escPressed")) {
				...
				if (oEvt.getSource().getParent().getBindingContext().getProperty("title")) {
					this.getView().getModel("appView").setProperty("/bookTitleEmpty", false);
				}
			}
		},
		...
		onSave: function () {
			...
			this.getView().getModel().submitBatch("booksGroup").then(fnSuccess, fnError);
		...
		onSearch: function () {
			...
			oView.byId("idBooksTable").getBinding("items").filter(oFilter, FilterType.Application);
		},
		...
		onSort: function () {
			...
			oView.byId("idBooksTable").getBinding("items")
